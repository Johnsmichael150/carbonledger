...
mod tests {
    ...
    // ── Issue #52: Resource optimization tests ────────────────────────────────

    /// Batch exceeding MAX_BATCH_SIZE must be rejected immediately.
    #[test]
    fn test_bulk_purchase_exceeds_max_batch_size_fails() {
        let env = Env::default();
        let (client, _, _, _) = setup(&env);
        let buyer = Address::generate(&env);

        // Build vectors with MAX_BATCH_SIZE + 1 entries
        let mut ids: soroban_sdk::Vec<String> = vec![&env];
        let mut amts: soroban_sdk::Vec<i128> = vec![&env];
        for i in 0..11_u32 {
            ids.push_back(s(&env, &format!("list-{i}")));
            amts.push_back(1_i128);
        }

        let result = client.try_bulk_purchase(&buyer, &ids, &amts);
